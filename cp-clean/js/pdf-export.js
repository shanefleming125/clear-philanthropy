...
async function exportToPDF() {
  ...
  ratios.forEach(({ label, value, rating }, i) => {
    const x  = M + (i%2) * (col+6);
    const ry = y + Math.floor(i/2) * 22;
    if (Math.floor(i/2) % 2 === 0) fill(x, ry-6, col, 21, [249,250,251]);
    sf('normal', 8.5, C.muted);
    doc.text(label, x+4, ry+4);
    sf('bold', 10, C.dark);
    doc.text(value, x + col - 30, ry+4);
    if (rating) {
      const rs = ratingStyle(rating);
      pill(x + col - 21, ry+4, rating, rs.bg, rs.fg);
    }
  });
  y += Math.ceil(ratios.length/2) * 22 + 8;
  ...
  if (docs.length) {
    ...
    y += 5; hline(y); y += 5;
    fill(M, y, W-M*2, 11, C.lightbg);
    sf('normal', 8.5, C.dark);
    const docText = doc.splitTextToSize(docs.join('   ·   '), W-M*2-8);
    doc.text(docText, M+4, y+7);
    y += 11 + docText.length * 5 + 5;
  }
  ...
  // OVERALL IMPRESSION
  if (impression) {
    if (y > 230) { doc.addPage(); addPageHeader(); y = 28; }
    sf('bold', 8.5, C.navy);
    doc.text('OVERALL IMPRESSION', M, y);
    y += 5; hline(y); y += 6;
    sf('normal', 8.5, C.dark);
    const lines = doc.splitTextToSize(impression, W-M*2);
    lines.forEach(l => { if (y > 268) { doc.addPage(); addPageHeader(); y = 28; } doc.text(l, M, y); y += 5.5; });
    y += 5;
  }
  ...
  if (hasAI) {
    ...
    y += 6; hline(y, C.blue); y += 8;
    sf('normal', 9.5, C.dark);
