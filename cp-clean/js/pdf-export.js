...
async function exportToPDF() {
  ...
  ratios.forEach(({ label, value, rating }, i) => {
    ...
    // Row bg
    if (Math.floor(i/2) % 2 === 0) fill(x, ry-3, col, 10, [249,250,251]);
    sf('normal', 8, C.muted);
    doc.text(label, x+3, ry+3);
    sf('bold', 9, C.dark);
    doc.text(value, x + col - 28, ry+3);
    if (rating) {
      const rs = ratingStyle(rating);
      pill(x + col - 20, ry+3, rating, rs.bg, rs.fg);
    }
  });
  y += Math.ceil(ratios.length/2) * 11 + 5;
  ...
  if (hasAI) {
    ...
    sf('normal', 9, C.dark);
    const aiLines = doc.splitTextToSize(aiText, W-M*2);
    aiLines.forEach(l => {
